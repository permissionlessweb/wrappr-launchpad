use crate::common_setup::contract_boxes::contract_wrappr_minter;
use crate::common_setup::contract_boxes::contract_wrappr721_base;
use crate::common_setup::contract_boxes::{contract_wrappr_factory, App};
...
use wrappr_factory::msg::WrapprMinterCreateMsg;
use wrappr_factory_utils::msg::{CollectionParams, WrapprFactoryExecuteMsg};
...
pub fn wrappr_minter_wrappr721_nt_code_ids(router: &mut App) -> CodeIds {
    let minter_code_id = router.store_code(contract_wrappr_minter());
    println!("wrappr_minter_code_id: {minter_code_id}");

    let factory_code_id = router.store_code(contract_wrappr_factory());
    println!("wrappr_factory_code_id: {factory_code_id}");

    let wrappr721_code_id = router.store_code(contract_wrappr721_base());
    println!("wrappr721_code_id: {wrappr721_code_id}");
    CodeIds {
        minter_code_id,
        factory_code_id,
        wrappr721_code_id,
...
pub fn wrappr_minter_wrappr721_collection_code_ids(router: &mut App) -> CodeIds {
    let minter_code_id = router.store_code(contract_wrappr_minter());
    println!("wrappr_minter_code_id: {minter_code_id}");
...
pub fn setup_minter_contract(setup_params: MinterSetupParams) -> MinterCollectionResponse {
    ...
    let wrappr721_code_id = setup_params.wrappr721_code_id;
    ...
    let factory_addr = router
        .instantiate_contract(
            factory_code_id,
            minter_admin.clone(),
            &wrappr_factory::msg::InstantiateMsg { params },
            &[],
            "factory",
            None,
        )
    ...
    msg.collection_params.code_id = wrappr721_code_id;
...
pub fn sudo_update_params(
    ...
    code_ids: CodeIds,
    update_msg: Option<wrappr_factory_utils::msg::UpdateMinterParamsMsg<Empty>>,
    ...
    for collection_response in collection_responses {
        let update_msg = match update_msg.clone() {
            ...
            None => wrappr_factory_utils::msg::UpdateMinterParamsMsg {
        ...
        let sudo_update_msg = wrappr_factory::msg::SudoMsg::UpdateParams(Box::new(update_msg));
...
pub fn configure_wrappr_minter(
    app: &mut App,
    minter_admin: Addr,
    ...
    for (index, collection_param) in collection_params_vec.iter().enumerate() {
        let setup_params: MinterSetupParams = MinterSetupParams {
            ...
            wrappr721_code_id: code_ids.wrappr721_code_id,
