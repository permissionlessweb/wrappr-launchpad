...
deploy-local:
	docker kill juno || true
	docker volume rm -f juno_data
	docker run --rm -d --name terp \
		-e DENOM=ujuno \
		-e CHAINID=testing \
		...
		-p 26657:26657 \
		-p 9090:9090 \
		--mount type=volume,source=juno_data,target=/root \
		cosmoscontracts/juno:20.0.0 /data/entry-point.sh $(TEST_ADDRS)
