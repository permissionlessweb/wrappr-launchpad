...
use cosmwasm_std::{
    coin, Addr, BankMsg, Binary, Empty, Event, StdError, StdResult, Timestamp, Uint128,Response,
    SubMsg,
};
use cw721::{
...
impl CollectionInfoResponse {
    pub fn print_external_link(
        &self,
        collection: Addr,
        res: &mut Response,
    ) -> StdResult<Option<String>> {
        if let Some(external_link) = self.external_link.as_ref() {
            Ok(Some(external_link.to_string()))
        } else {
        Ok(None)
        }
    }
