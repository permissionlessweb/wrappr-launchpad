...
use crate::state::{increment_token_index, Config, COLLECTION_ADDRESS, NATIVE_DENOM, CONFIG, STATUS};
use wrappr_factory::msg::{BaseMinterCreateMsg, ParamsResponse};
...
use cosmwasm_std::{
    ...
    StdResult, Timestamp, WasmMsg,Response,SubMsg
};
use cw2::set_contract_version;
use cw_utils::{must_pay, nonpayable, parse_reply_instantiate_data};
use wr2::query::Wr2QueryMsg;
use wr4::{QueryMsg, Status, StatusResponse, SudoMsg};
use wrappr_utils::{ExecuteMsg as Cw721ExecuteMsg, InstantiateMsg as Cw721InstantiateMsg};
use cw_wrappr::msg::{CollectionInfoResponse, QueryMsg as Cw721QueryMsg};
use url::Url;
...
pub fn instantiate(
    ...
    let factory_params: ParamsResponse = deps
        .querier
        .query_wasm_smart(factory.clone(), &Wr2QueryMsg::Params {})?;
...
pub fn execute_mint_sender(
    ...
    // allow only wrappr creator address to mint
    if collection_info.creator != info.sender {
        return Err(ContractError::Unauthorized(
            "Sender is not wrappr creator".to_owned(),
        ));
    };
    ...
    let factory: ParamsResponse = deps
        .querier
        .query_wasm_smart(config.factory, &Wr2QueryMsg::Params {})?;
...
//         msg: to_binary(&Cw721ExecuteMsg::<Extension, Empty>::UpdateStartTradingTime(start_time))?,
//         funds: vec![],
//     };
...
// Reply callback triggered from wrappr contract instantiation in instantiate()
#[cfg_attr(not(feature = "library"), entry_point)]
pub fn reply(deps: DepsMut, _env: Env, msg: Reply) -> Result<Response, ContractError> {
    ...
    match reply {
        Ok(res) => {
            ...
            Ok(Response::default()
                .add_attribute("action", "instantiate_wrappr_reply")
                .add_attribute("wrappr_address", collection_address))
        }
