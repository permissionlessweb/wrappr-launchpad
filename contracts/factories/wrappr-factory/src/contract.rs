...
const CONTRACT_NAME: &str = "crates.io:cw-wrappr-factory";
...
pub fn update_params<T, C>(
    ...
    if let Some(add_wrappr721_code_ids) = param_msg.add_wrappr721_code_ids {
        for code_id in add_wrappr721_code_ids {
            params.allowed_wrappr721_code_ids.push(code_id);
        }
    }
    params.allowed_wrappr721_code_ids.dedup();
    if let Some(rm_wrappr721_code_ids) = param_msg.rm_wrappr721_code_ids {
        for code_id in rm_wrappr721_code_ids {
            params.allowed_wrappr721_code_ids.retain(|&x| x != code_id);
        }
    }
