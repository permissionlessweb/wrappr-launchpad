...
use cosmwasm_std::{
    ensure, ensure_eq, to_binary, Binary, Deps, DepsMut, Env, MessageInfo, StdResult, WasmMsg,Response,
};
use cw2::set_contract_version;
use cw_utils::must_pay;
// use sg1::checked_fair_burn;
use wr2::msg::UpdateMinterParamsMsg;
use wr2::query::{AllowedCollectionCodeIdResponse, AllowedCollectionCodeIdsResponse, Wr2QueryMsg};
...
use crate::state::{SUDO_PARAMS, NATIVE_DENOM};

// version info for migration info
const CONTRACT_NAME: &str = "crates.io:wrappr-factory";
...
#[cfg_attr(not(feature = "library"), entry_point)]
pub fn query(deps: Deps, _env: Env, msg: Wr2QueryMsg) -> StdResult<Binary> {
    match msg {
        Wr2QueryMsg::Params {} => to_binary(&query_params(deps)?),
        Wr2QueryMsg::AllowedCollectionCodeIds {} => {
            to_binary(&query_allowed_collection_code_ids(deps)?)
        }
        Wr2QueryMsg::AllowedCollectionCodeId(code_id) => {
